...
//    DLog(@">>>scanForPeripheralsNotifyReceived");
}
...
//    DLog(@">>>didDiscoverPeripheralNotifyReceived:%@",peripheral.name);
}

//开始连接设备
-(void)connectToPeripheralNotifyReceived:(NSNotification *)notify{
//    DLog(@">>>connectToPeripheralNotifyReceived");
}
...
//    DLog(@">>>外设连接断开连接 %@: %@\n", [peripheral name], [error localizedDescription]);
    if (error)
    {
        DLog(@">>> didDisconnectPeripheral for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    ...
    if ([self findConnectedPeripherals].count == 0) {
        ...
        //    DLog(@">>> stopConnectAllPerihperals");
    }
}
...
//  DLog(@">>>扫描到服务：%@",peripheral.services);
    if (error)
    {
        DLog(@">>>didDiscoverServices for %@ with error: %@", peripheral.name, [error localizedDescription]);
//        return;
    }
    //回叫block
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    if (error)
    {
        DLog(@"error didDiscoverCharacteristicsForService for %@ with error: %@", service.UUID, [error localizedDescription]);
//        return;
    }
    //回叫block
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    if (error)
    {
        DLog(@"error didUpdateValueForCharacteristic %@ with error: %@", characteristic.UUID, [error localizedDescription]);
//        return;
    }
    //查找字段订阅
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    if (error)
    {
        DLog(@"error Discovered DescriptorsForCharacteristic for %@ with error: %@", characteristic.UUID, [error localizedDescription]);
//        return;
    }
    //回叫block
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error{
    ...
    if (error)
    {
        DLog(@"error didUpdateValueForDescriptor  for %@ with error: %@", descriptor.UUID, [error localizedDescription]);
//        return;
    }
    //回叫block
...
//    DLog(@">>>uuid:%@,new value:%@",descriptor.UUID,descriptor.value);
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    DLog(@">>>uuid:%@,isNotifying:%@",characteristic.UUID,characteristic.isNotifying?@"isNotifying":@"Notifying");
...
//    DLog(@"peripheralDidUpdateRSSI -> RSSI:%@",RSSI);
...
//    DLog(@"peripheralDidUpdateRSSI -> RSSI:%@",peripheral.RSSI);
//        if ([currChannel blockOnDidReadRSSI]) {
